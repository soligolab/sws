...
pub async fn run(
    ...
    let entity_map: HashMap<String, EntityMapping> = cfg
        .entities
        .iter()
...
async fn run_session(
    ...
    match client
        ...
        .await
    {
        Ok(resp) if resp.status().is_success() => match resp.json::<Vec<HaState>>().await {
            Ok(states) => {
                for state in states {
    ...
    let first_json: Json = serde_json::from_str(
        first
            .to_text()
    ...
    let auth_json: Json = serde_json::from_str(
        auth_resp
            .to_text()
            .map_err(|e| anyhow::anyhow!("WS non-text frame: {e}"))?,
    )?;
    ...
    let writable: Vec<(String, EntityMapping)> = cfg
        .entities
        .iter()
    ...
    let tag_to_entity: HashMap<String, EntityMapping> = writable.into_iter().collect();
...
async fn handle_event(text: &str, db: &Arc<TagDb>, entity_map: &HashMap<String, EntityMapping>) {
    ...
    let Some(mapping) = entity_map.get(entity_id) else {
        return;
    };

    let new_state = &data["new_state"];
    let Some(state_str) = new_state["state"].as_str() else {
        return;
    };
...
fn effective_token(cfg: &HomeAssistantConfig) -> anyhow::Result<String> {
    if let Some(env_var) = &cfg.token_env {
        std::env::var(env_var).map_err(|_| anyhow::anyhow!("env var '{}' not set", env_var))
    } else if let Some(t) = &cfg.token {
        Ok(t.clone())
    } else {
        anyhow::bail!(
            "HA source '{}': neither 'token' nor 'token_env' is configured",
            cfg.id
        )
    }
}
