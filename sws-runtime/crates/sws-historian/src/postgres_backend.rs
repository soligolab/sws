...
impl PostgresBackend {
    ...
    pub async fn record(&self, tag_id: &str, sample: &Sample) {
        ...
        let Some(mut g) = self.get_client().await else {
            return;
        };
    ...
    pub async fn stats(&self) -> DatastoreStats {
        ...
        let (sc, tc, old, new, sz) = async {
            let sc: i64 = client
                .query_one("SELECT COUNT(*) FROM sws_samples", &[])
                .await
                .ok()
                .map(|r| r.get(0))
                .unwrap_or(0);
            let tc: i64 = client
                .query_one("SELECT COUNT(DISTINCT tag_id) FROM sws_samples", &[])
                .await
                .ok()
                .map(|r| r.get(0))
                .unwrap_or(0);
            let old: Option<i64> = client
                .query_one("SELECT MIN(ts_ms) FROM sws_samples", &[])
                .await
                .ok()
                .and_then(|r| r.try_get(0).ok());
            let new: Option<i64> = client
                .query_one("SELECT MAX(ts_ms) FROM sws_samples", &[])
                .await
                .ok()
                .and_then(|r| r.try_get(0).ok());
            let sz: Option<i64> = client
                .query_one("SELECT pg_total_relation_size('sws_samples')", &[])
                .await
                .ok()
                .map(|r| r.get(0));
            (sc, tc, old, new, sz)
    ...
    pub async fn purge(
        ...
        if let Some(days) = retention_days {
            ...
            deleted += client
                .execute("DELETE FROM sws_samples WHERE ts_ms < $1", &[&cutoff])
                .await
                .unwrap_or(0);
        }
...
async fn connect_and_init(connect_str: &str, schema: &str) -> anyhow::Result<Client> {
    ...
    client
        ...
        .await
        .map_err(|e| anyhow::anyhow!("search_path: {e}"))?;
    client
