...
use rseip::client::ab_eip::{AbEipClient, TagType, TagValue};
use rseip::precludes::*;

use sws_core::{
    EnIpConfig, EnIpDataType, EnIpTagMapping, TagDb, TagQuality, TagValue as SwsTagValue,
...
pub async fn run(
    ...
    if let Err(e) = session(&cfg, &db, &mut write_rx, cancel).await {
        ...
        for tm in &cfg.tags {
            db.ingest(tm.tag.clone(), SwsTagValue::Float(0.0), TagQuality::Bad)
                .await;
        }
    }
}
...
async fn session(
    ...
    let write_map: HashMap<String, usize> = cfg
        .tags
        .iter()
        .enumerate()
