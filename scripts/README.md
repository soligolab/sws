...
| `check_sonda.sh` | la sonda del dispositivo (Q52, `deploy/container/sonda-dispositivo.sh`) smette di girare con `sh -s` — bashismi, un comando che legge stdin, righe che non sono `SONDA chiave=valore`; la si esegue su questo PC come file e da stdin |
| `check_password_browser.sh` | una password torna in `localStorage`/`sessionStorage` (chiave o valore che nomini pass/pwd), `SavedDevice` riacquista il campo `pass`, o la pulizia delle password vecchie sparisce dall'avvio — fino al 2026-09-09 l'IDE ne salvava tre famiglie in chiaro |
| `check_barra_allarmi.sh` | `alarm_banner` torna a comportarsi in due modi: la funzione `nellaBarra` (web, `AlarmBanner.tsx`) e `nella_barra` (LVGL, `lvgl_render.rs`) smettono di nominare gli stessi stati ISA, o `alarm_bell`/`alarm_viewer` lasciano il filtro su `active` — fino all'11-09-2026 il web mostrava i non confermati e il pannello gli attivi |
| `check_target_progetto.sh` | cambiare il motore di rendering dall'IDE smette di arrivare al pannello: la rotta `PUT /api/project/target` risponde 204 e scrive `project.yaml`, ma `display-target` resta quello di prima — e lo schermo del pannello non commuta. Prova anche che «web» si scriva come **assenza** del campo e che un motore sconosciuto sia rifiutato senza toccare il progetto. Vuole un runtime: lo avvia da sé |
| `check_i18n_parita.sh` | il web e il viewer LVGL traducono due insiemi DIVERSI di campi, e il difetto si vede solo in campo: un progetto tradotto bene mostra `{{chiave}}` sul pannello e il testo giusto nell'IDE. Estrae le due liste dal codice (`projectI18n.ts` e `lvgl_render.rs`) e le confronta; tiene anche il conto dichiarato del testo visibile che non traduce nessuno dei due. Il 15-09-2026 le liste divergevano di dieci campi, sette dei quali il pannello disegna davvero |
| `check_testi_sistema.sh` | il testo di sistema — «Ora», «sì», «N/D», il titolo di una notifica — torna a divergere fra il pannello LVGL, le notifiche e il viewer web. La fonte è `tests/fixtures/testi-sistema.json`; la guardia verifica che ogni nome di voce e ogni parola tradotta compaiano tali e quali in `sws-core/src/testi_sistema.rs` e in `sws-editor/src/i18n/testiSistema.ts`, e che `viewerChrome.*` non sia tornato nei cataloghi dell'IDE. Fino al 18-09-2026 il web aveva otto di queste parole in due lingue e seguiva la lingua dell'IDE: un tedesco leggeva «Zeit» sul vetro e «Time» nel browser |
| `check_colori.sh` | i colori predefiniti tornano a vivere in quattro posti. La fonte è `tests/fixtures/colori-predefiniti.json` (specchiata in `sws-editor/src/coloriPredefiniti.ts` e in `lvgl_render.rs`, confrontate dai test); la guardia verifica che la creazione (`oggettiNuovi.ts`) e i template non contengano `var(--`, che nessuna chiamata `colorInput` del pannello porti un ripiego proprio, che ogni `<input type="color">` di `EditorShell.tsx` riceva sei cifre hex via `estraiHex()`/`CampoColore`, e che ogni voce della fixture compaia nel modulo TS. Fino al 21-09-2026 un Testo nasceva bianco sul canvas e nero nel pannello |
| `check_tipi_scalari.sh` | i tipi scalari delle variabili (D5: bool, i8…i64, u8…u64, f32/f64, string(N), datetime, con `int`/`float` alias) tornano a vivere in elenchi separati. La fonte è `tests/fixtures/tipi-scalari.json`; la guardia verifica che ogni nome compaia in `sws-core/src/tipo.rs` e in `sws-editor/src/tag/tipiScalari.ts` nello stesso ordine, che nessuna `<select>` rielenchi i tipi a mano (passano da `<OpzioniTipo />`) e che `validate.rs` non abbia più la costante `TIPI_DATO` |
| `check_tag_refs.sh` | `CAMPI_TAG` (`validate.rs`) e `TAG_FIELDS` (`collectTagIds.ts`) sono due elenchi a mano degli stessi campi-tag scalari di un oggetto sinottico; il 21-09-2026 divergevano di quattro campi (`motion_tag`, `pipe_flow_tag`, `symbol_spin_tag`, `gauge_sp_tag`), validati lato server ma non sottoscritti lato client — l'oggetto riceveva lo snapshot iniziale del tag e poi si congelava, senza nessun errore. La guardia confronta i due elenchi e verifica che ogni collezione di `CAMPI_TAG_COLLEZIONE` (trend_tags, xy_series, table_rows, bar_series, pie_slices) sia citata anche in `collectTagIds.ts` |
| `check_segreti.sh` | un segreto di progetto torna fuori dal recinto di `secrets.yaml`. Quattro classi, tutte viste davvero: (1) un **campo segreto nuovo dimenticato** — ogni campo di `project.rs` il cui nome somiglia a `pass|token|secret|key|pwd|connection_string` dev'essere in `CAMPI_COPERTI` di `segreti.rs` o in un'eccezione motivata (`*_env` è il nome di una variabile d'ambiente, non il suo valore); (2) i punti che nominano `project.yaml` fuori dai test sono un **elenco chiuso con un tetto per file**, così chi ne aggiunge uno deve dire come ci passano i segreti; (3) nessuna macro di log interpola una variabile che si chiama token/password/secret, e chi compone l'URL di Telegram (`/bot{…}`) deve nominare `redigi` nello stesso file — fino al 2f bastava un DNS che non risolveva per stampare il bot token nei log, perché `reqwest::Error` si porta dietro l'URL; (4) nessun template porta credenziali, che si copierebbero in ogni progetto nuovo |
| `check_segreti_e2e.sh` | **(con stack)** i segreti provati invece che letti: due runtime veri, un token salvato dall'API, e poi dieci domande a cui il codice non può rispondere da solo — il token è in `secrets.yaml` e non in `project.yaml`, il file è 0600 di qua e di là, la GET lo maschera, l'export normale non lo porta (e **nessun file** dello zip lo contiene, non solo `secrets.yaml`), `?segreti=1` sì, il deploy lo consegna, un deploy senza segreti non svuota il dispositivo, un progetto vecchio si migra col backup prima e non rimigra alla seconda apertura, i log non lo nominano. Provata rossa il 23-09-2026 con due falsificazioni sul codice vero; la prima ha mostrato che i permessi sul dispositivo li scrive `upload_project_zip` e non `scrivi_atomico_0600`, due strade che possono divergere — per questo si controllano entrambe |
| `check_i18n_ui.sh` | l'interfaccia dell'IDE torna a parlare italiano cablato invece che dal catalogo. `it.json`/`en.json` erano in parità perfetta e protetta da un test, e nonostante questo il 18-09-2026 c'erano **~640 stringhe italiane fuori da `t()`** (394 in `ConfigView.tsx`) e 35 `confirm/alert/prompt` con testo letterale: la parità era guardata, la copertura no. Euristica dichiarata (accenti, parole forti, due deboli distinte), esclusioni motivate una per una — fra cui il **contenuto predefinito degli oggetti** letto da `TEXT_FIELDS` di `projectI18n.ts`, che è asse (b) e non va in `t()` — tetti per file che possono solo scendere, `--elenca <file>` per pagare il debito, `--autotest` che inchioda l'euristica |
| `check_release_coerente.sh` | una release resta a metà: la versione dichiarata nei quattro file non coincide, il `CHANGELOG.md` non racconta la versione che il codice dice di essere, una sezione rilasciata non ha un tag, o **un tag di versione esiste solo in questo checkout**. Nasce dalla notte fra il 16 e il 17-09-2026: due macchine sullo stesso progetto, `chore(release): 2.8.0` pushato dall'una e il tag `2.8.0` rimasto nell'altra, con la sessione di casa che chiudeva dicendo «niente da pushare» — vero nel suo checkout, falso nel repo. Scrivendola è saltato fuori che anche la `2.7.3`, rilasciata il 12-09, non è mai stata taggata: è nel debito dichiarato dentro la guardia, in attesa di una decisione |
| `check_primo_utente.sh` | definire il primo utente chiude fuori chi lo sta definendo: sull'IDE scrivere `users.yaml` accende l'autenticazione nello stesso runtime che serve l'editor, e con un solo Operator il progetto diventa irraggiungibile (guasto vero del 14-09-2026). Prova la catena su due runtime veri — un IDE, che deve restare aperto, e un dispositivo, dove il primo account dev'essere un Admin. Li avvia da sé |
| `check_via_di_fuga.sh` | qualcuno ruba lo schermo al launcher Pixsys in modalità configurazione: `sws-display-apply.sh` tocca lo schermo prima di guardare la modalità, perde il ciclo d'attesa, commuta «nel dubbio», comanda `chromium@wp-control`, abilita `weston`, sposta il backend fuori da `default.target`, perde il ripiego D-Bus o imposta l'URL dopo lo start — i vincoli 1-7 del piano del 2026-09-03 |
...
| `check_tipo_scrittura.sh` | una scrittura col tipo sbagliato torna a passare: fino a Q27 una stringa su un tag `bool` prendeva 204 e arrivava al plugin, cioè al PLC. Prova end-to-end i rifiuti (400 col motivo) e che la coercizione senza perdita scriva **il tipo dichiarato** — il GET deve restituire `true`, non la stringa `"true"` |
| `check_ricette.sh` | un Operator torna a scrivere tag protetti Admin via ricetta, l'all-or-nothing si rompe (ricetta applicata a metà), o l'apply smette di firmare l'audit. Si porta il suo progetto con ricette e `write_min_role`, perché nessun template li ha e nessun'altra guardia vedrebbe la regressione |
| `check_mqtt_topic_vuoto.sh` | una riga MQTT senza topic torna a raggiungere il disco o il broker. Nasce da Sandokan (2026-09-07): una riga vuota lasciata dallo sfoglia-broker faceva mandare una sottoscrizione con filtro a lunghezza zero, mosquitto chiudeva la connessione 2 ms dopo e morivano **tutti** e 28 i topic della sorgente — in loop ogni 5 s, con «Broken pipe» come unico indizio. Prova la potatura al salvataggio (quindi prima del deploy) e i rilievi del validatore |
| `check_chiave_host.sh` | il deploy torna a fermarsi senza spiegazioni quando un dispositivo cambia identità (factory reset), o l'endpoint che dimentica la vecchia chiave accetta un host che non dovrebbe — finisce in `ssh-keygen -R`, e un nome che comincia per `-` diventerebbe un'opzione. Gira con una `HOME` finta: non tocca il `known_hosts` di chi la lancia |
| `check_connessione_remota.sh` | connettersi a un dispositivo **senza utenti** torna a chiudere fuori chi lavora. Il 401 del pannello risaliva fino al client, che lo legge come «la TUA sessione è scaduta»: comparivano insieme «unauthorized» e il modale di riautenticazione, su una sessione viva e per un utente la cui password non poteva funzionare. Gira con due runtime veri, uno con utenti e uno senza |
