# Domande chiuse — decise E realizzate, verificate prima di archiviare

> Vengono da `docs/OPEN_QUESTIONS.md`, dove restano indicizzate per numero: i rimandi «Qn» di
> `STATUS.md` e degli altri documenti continuano a risolvere. **I numeri non si riusano mai.**
>
> Una scheda arriva qui solo se (1) la decisione è del maintainer, (2) il corpo dichiara il lavoro
> fatto, e (3) l'affermazione è stata **verificata sul codice** il giorno dell'archiviazione — il
> timbro in testa a ciascuna dice quale prova è stata trovata. La verifica non è pignoleria: il
...
> stata **verificata sul codice** prima di archiviare: il crate `sws-pyscript` con PyO3 esiste, e `router.rs:1315` esegue gli script in RestrictedPython con `safe_builtins`.
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: `prost` è in `sws-plugin-mqtt/Cargo.toml:16` (Protobuf a mano, opzione C).
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: sei crate `sws-plugin-*` compilati nel workspace; zero occorrenze di `libloading`/`dlopen`.
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: `zustand` in `package.json:30`; la decisione è promossa in `docs/adr/0001-state-management.md`.
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: `i18next` in `package.json:26`, scaffolding vivo (T-40 tabella lingue).
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: `SYMBOLS` in `sws-editor/src/symbols/library.tsx:605`, 40 voci.
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: `LICENSE` è la GNU AGPL v3.
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: tutti e tre i deploy partono con `--no-admin` (verificato il 2026-09-06 su generic-linux, yocto e container); E/F rinviate al product phase per decisione.
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: 23 `deny_unknown_fields` in `router.rs` e il validatore `unknown_fields` in `validate.rs` con i suoi test.
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: `merge_preserved` in `router.rs` conserva le chiavi sconosciute a ogni riscrittura (riverificato lavorando a Q30 il 2026-09-05).
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: `secondary?`/`accent?` in `BrandColors` (`branding/index.ts`), col commento che cita questa scheda.
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: `BrandNeutrals` override per-brand e per-variante (`branding/index.ts`), col commento che cita questa scheda.
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: i 15 Decided interni sono tutti realizzati; `SUPPORTED_TYPES` conta oggi 35 tipi (nota di freschezza del 2026-09-05 in coda alla scheda) e `check_lvgl_types.sh` tiene il conto.
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: `--synoptic-text` derivato dallo sfondo pagina in `SvgCanvas.tsx:1538` e i test Q18 in `lvgl_render.rs`: implementata su entrambi i motori (riverificato il 2026-09-05, quando `check_f7` è stata corretta per misurare il token giusto).
...
**Decided**: sì — vedi il blocco in testa (2026-08-25). Questa riga diceva ancora «not yet»
perché il piano che aggiunse la decisione in testa non aggiornò la coda: la scheda ha detto due
cose opposte per dodici giorni. Verificato il 2026-09-06 che l'implementazione c'è su entrambi i motori.
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: `drm_backend_blocker` in `main.rs` rifiuta il backend sui PixsysOS con il messaggio che spiega perché — esattamente la decisione; e dal 2026-09-06 il mismatch pagina/display non manda più in panic (F2).
...
**Decided**: sì — vedi il blocco in testa (2026-08-25). La coda diceva ancora «not yet» per lo
stesso difetto d'edizione di Q18. Verificato il 2026-09-06: `drm_backend_blocker` fa esattamente
ciò che la decisione chiede.
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: il segnale «il progetto è cambiato, rileggi» esiste (`client.rs:496`) e le navigazioni stampano `[reload] … ridisegnata`.
...
**Stato**: realizzata — questo marcatore diceva ancora «aperta» per lo stesso difetto
d'edizione di Q18/Q19 (il piano del 2026-08-25 decise in testa senza toccare la coda).
Verificato il 2026-09-06. Emersa il 2026-08-24, segnalata indirettamente dal maintainer.
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: la sezione Python unica esiste: «Colonna sinistra: TUTTO il Python del progetto (Q21)» in `ConfigView.tsx:6985`.
...
**Stato**: realizzata — questo marcatore diceva ancora «aperta» per lo stesso difetto
d'edizione di Q18/Q19 (il piano del 2026-08-25 decise in testa senza toccare la coda).
Verificato il 2026-09-06. Domanda del maintainer, 2026-08-24: «gli script python sarebbe bello fossero
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: la patch `patches/lvgl/0001-init-x_ext_buf_assigned.patch` esiste e `check_vendor_patches.sh` la copre; la segnalazione a monte fu scartata esplicitamente (si è proceduto subito).
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: `LV_USE_FREETYPE 1` in `lv_conf/lv_conf.h:672`; provato sul WP630 (2.567+375 avvisi di glifo → zero).
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: `system_stop` disarma prima di fermare (`system.rs`), e i pulsanti RUN/STOP sono in testata (`RuntimeCtrl.tsx`).
...
> **Archiviata il 2026-09-06** — decisione del maintainer realizzata, e l'affermazione è
> stata **verificata sul codice** prima di archiviare: `sws-web/src/cron.rs` esiste, il validatore lo chiama (`cron_rilievi`), e la spia in testata è documentata nel manuale dal 2026-09-06.
...
> **Archiviata il 2026-09-12** — decisione del maintainer realizzata, e l'affermazione è stata **verificata sul codice** prima di archiviare: il render dei simboli builtin esiste (`fn render_symbol` in `lvgl_render.rs:6668`, con pump/valve/motor/tank disegnati a mano su `lv_canvas`).
...
> **Archiviata il 2026-09-12** — decisione del maintainer realizzata, e l'affermazione è stata **verificata sul codice** prima di archiviare: `display_target.rs` (`sws-web/src/`) e `deploy/container/sws-display-apply.sh` esistono entrambi, la commutazione web/LVGL guidata dal progetto è in codice.
...
> **Archiviata il 2026-09-12** — decisione del maintainer realizzata, e l'affermazione è stata **verificata sul codice** prima di archiviare: `AppState::project_write_lock` (`router.rs:173`) e il meccanismo `If-Match`/`calcola_impronta` sull'hash del solo `project.yaml` (`router.rs:2550-2613`) esistono.
...
> **Archiviata il 2026-09-12** — decisione del maintainer realizzata, e l'affermazione è stata **verificata sul codice** prima di archiviare: `/api/script/exec` non è più montata su nessun router — restano solo i commenti che ne ricordano la rimozione (`router.rs:1026,1700,4289`, tutti che citano Q47).
...
> **Archiviata il 2026-09-12** — decisione del maintainer realizzata, e l'affermazione è stata **verificata sul codice** prima di archiviare: `sws-web/src/deploy.rs` non esiste più nel checkout, nessuna rotta `/api/deploy/remote` registrata in `router.rs`.
...
> **Archiviata il 2026-09-12** — decisione del maintainer realizzata, e l'affermazione è stata **verificata sul codice** prima di archiviare: `sws-web/src/dispositivi.rs` esiste e `GET/PUT /api/devices` è montata (`router.rs:418`).
...
> **Archiviata il 2026-09-12** — decisione del maintainer realizzata, e l'affermazione è stata **verificata sul codice** prima di archiviare: `GET /api/build/stato` è montata (`router.rs:379`, `crate::packaging::stato_build`).
...
> **Archiviata il 2026-09-12** — decisione del maintainer realizzata, e l'affermazione è stata **verificata sul codice** prima di archiviare: `sws-web/src/sonda.rs` esiste e `POST /api/device/probe` è montata (`router.rs:406`).
...
> **Archiviata il 2026-09-12** — decisione del maintainer: opzione 1, fermarsi qui. Nessun
> codice segue — il validatore resta con il solo rilievo di T-52 («N oggetti fuori pagina»),
> verificato che sia già lo stato attuale di `sws-web/src/validate.rs` prima di archiviare.
...
**Default for PoC**: opzione 1. **Decided (2026-09-12, maintainer)**: opzione 1 — fermarsi qui.

---

## Q32 — Dove deve vivere il progetto che si sta modificando?

> **Archiviata il 2026-09-12** — decisione del maintainer: opzione 1, basta il marcatore già
> presente in testata. Nessun codice segue, per nessuno dei due casi rimasti (dispositivo con
> `SWS_ENABLE_IDE=1`, `start_runtime.sh` locale) — verificato che sia già lo stato attuale
> prima di archiviare.
...
3. **Presa diretta = sola lettura per default**, e per modificare si fa un pull sull'editor locale,
...
**Decided (2026-09-12, maintainer)**: opzione 1 — basta il marcatore.

---

## Q31 — La chat non funziona quando l'IDE è collegato a un runtime remoto, e non è chiaro cosa dovrebbe fare

> **Archiviata il 2026-09-12** — verificata sul codice il 2026-09-01, e **verificata dal vivo**
> il 2026-09-12 prima di archiviare: due runtime di prova isolati (un "device" no-auth e un
> "editor" collegato ad esso via `POST /api/remote/connect`), un browser vero contro l'editor.
> Confermati tutti e quattro i punti della scheda: (1) l'avviso «l'assistente lavora sul
> progetto locale» compare nel pannello quando `remoteConnected`; (2) una richiesta che tocca
> il progetto produce una proposta che cita il tag/pagina del progetto **locale**
> (`e2e-chat-ai`/«Indicatori»), non quelli del device collegato (progetto `casa-locale`
> deliberatamente diverso, per accorgersi di uno scambio); (3) il socket si vede aprire su
> `ws://.../ws/ai` (locale), mai su `/ws/remote/ai`; (4) nessuna risposta 404 su alcun canale
> WebSocket, nessun loop di riconnessione. Un primo giro del collaudo aveva dato un falso
> negativo sull'avviso — sistemato capendo che `remoteConnected` nello store si sincronizza da
> `/api/remote/status` solo al montaggio di `RuntimeConnectionTab` (ConfigView → Runtime), non
> all'avvio dell'app: chi collega il runtime e poi ricarica la pagina senza mai passare da lì
> vedrebbe lo stesso comportamento — non è un difetto di questa Q, è comportamento noto e non
> era in discussione qui.
...
> **Archiviata il 2026-09-12** — decisa e realizzata il 2026-09-09, **verificata dal vivo** il
> 2026-09-12 prima di archiviare: un'istanza di prova con `--projects-root` su una cartella
> scratch, una radice-sorella con un file marcatore e un link simbolico dentro la radice che
> punta a quella cartella. Tutti i tentativi di uscita sono stati respinti con 400
> ("fuori dalla cartella dei progetti"): `browse-dirs` su un path assoluto fuori radice (`/etc`),
> su la cartella genitore della radice, e seguendo il link simbolico; `mkdir` con
> `parent` che risale (`../../etc`) e `mkdir` dentro il link simbolico. Una `mkdir` legittima
> dentro la radice è riuscita (201) e nulla è comparso nella cartella fuori radice. In più,
> corretto un commento ormai falso sopra `browse_dirs` (`sws-web/src/projects.rs:1211-1217`):
> diceva ancora "no whitelist by design, free navigation" e citava un fallback a `$HOME` mai più
> vero da quando Q46 ha aggiunto `dentro_radice` — non un comportamento, solo la sua descrizione.
...
> **Archiviata il 2026-09-12** — decisa dal maintainer (scala per serie, tranne in
> `bar_mode: stacked`) e **realizzata e mergiata su `main`** (`f8e7fc6`), **verificata dal
> vivo** prima di archiviare: un progetto di prova isolato con un `bar_chart` a tre serie (due a
> default 0..100, una a 0..500 dichiarata) — screenshot del browser e istantanea
> `sws-lvgl-viewer --istantanea` sulla stessa pagina mostrano le stesse proporzioni relative fra
> le barre nei due motori, e LVGL segnala a log la stessa soppressione delle soglie che ora fa
> anche il web. LVGL non ha richiesto alcuna modifica: `BarChartSeries.min/max` e il default
> `0..100` esistevano già. Gate: cargo check/test, tsc, pnpm build, 348 vitest,
> check_lvgl_parity/check_lvgl_types/check_demo_templates. Referto completo in
> `docs/archive/2026-09-12-q28-scala-bar-chart.md`.
...
`istantanea_pagina` (T-51, fase 3) potrebbe chiuderla quando un modello la userà davvero.
...
> **Archiviata il 2026-09-12** — decisa dal maintainer (`write_data_type` su `TagDef`, accanto a
> `data_type`) e **realizzata e mergiata su `main`** (`dab5ef8`), **verificata dal vivo** prima
> di archiviare: round-trip YAML → API su un'istanza di prova con un progetto `casa-locale`
> (`write_data_type: string` sopravvive sui quattro tag delle tapparelle), screenshot
> dell'editor con "Tipo in scrittura: Stringa" nella riga avanzata. Il test
> `i_template_non_hanno_errori` passa **senza più `ECCEZIONI_NOTE`** — prova diretta che il campo
> risolve davvero i dodici casi, non solo che non rompe altro. Gate: cargo check/test/clippy/fmt
> sull'intero workspace, tsc, pnpm build, 348 vitest, 17/17 guardie statiche. Referto completo in
> `docs/archive/2026-09-12-q29-tag-due-tipi.md`.
...
> **Archiviata il 2026-09-13** — decisione del maintainer (opzione 3, «farlo funzionare», estesa
> anche ai 13 simboli mancanti trovati nella stessa scheda): **verificata sul codice e dal vivo**.
> `svg_assets::VENDORED` è vuota (`sws-runtime/crates/sws-lvgl-viewer/src/svg_assets.rs`), i 13
> mancanti + gli 11 importati sono tutti `kind: "builtin"` in `sws-editor/src/symbols/
> library.tsx` con un ramo in `draw_symbol` (`lvgl_render.rs`). I 4 SWS-originali (reactor,
> heat_exchanger, filter, separator) sono porting fedeli della stessa geometria; i 7 MDI più
> complessi (solar_panel, battery, transmission_tower, home_lightning, garage, window_open,
> roller_shade) sono ridisegni stilizzati — stesso concetto riconoscibile, non lo stesso
> contorno, per evitare una scomposizione in poligoni convessi stimata a circa una settimana.
> Collaudato dal vivo sul TC620 (non solo istantanea locale): tutti e 24 i simboli riconoscibili
> e ricolorabili da grigio a verde sullo schermo fisico. `main`, commit `2d09508`.
...
**Default for PoC**: opzione 4 — il manuale lo dice da oggi. **Decided**: 2026-09-13, opzione 3.

---
## Q45 — Il container di produzione non riparte dopo un reboot senza un permesso che l'utente finale non ha

> **Archiviata il 2026-09-13** — decisione del maintainer (opzione 1) dopo una misura dal vivo
> sul TC620 (Pixsys OS 2.1.1): **verificata sul codice e sul dispositivo**. `loginctl
> enable-linger`/`disable-linger` per il PROPRIO utente riescono senza `sudo`, in entrambe le
> direzioni — non per una regola Pixsys, ma per l'azione polkit di systemd
> `org.freedesktop.login1.set-self-linger` (`allow_any: yes` di default in
> `/usr/share/polkit-1/actions/org.freedesktop.login1.policy`), distinta da `set-linger` (il
> linger di un ALTRO utente, quella sì riservata a root). Il timore originale della scheda non
> si verifica su questo hardware. `deploy/container/install-container.sh` non avvisa più e
> continua: se il passo fallisce ora si ferma con un errore, perché un fallimento indicherebbe
> qualcos'altro di rotto sul dispositivo (systemd/polkit più vecchi o diversi), non un permesso
> mancante da aggirare. `main`, commit `067d7a2`.

Contenuto (misura e disegno del passo 0) in
[`docs/archive/2026-09-12-q45-linger-permesso-produzione.md`](../archive/2026-09-12-q45-linger-permesso-produzione.md).
