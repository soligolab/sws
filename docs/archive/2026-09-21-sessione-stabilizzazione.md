...
| 2 | navigatore web e LVGL | **✓ anche sul pannello.** I quattro casi (barra in fondo, colonna verticale, `children_of_current` con le sorelle e il percorso, pagina esclusa da un navigatore e raggiunta da un altro) sono stati renderizzati **dal binario aarch64 dentro il container del WP630**, con `podman exec … --istantanea`, e le immagini coincidono con quelle locali. **Poi, lo stesso giorno, ci è andato anche a schermo**, e senza fermare Weston: basta che il progetto dichiari `target: lvgl` e la unit di commutazione già installata fa tutto. Quello che qui si dava per impossibile era sbagliato su una premessa — il viewer non usa il framebuffer via DRM, ma `SDL_VIDEODRIVER=x11` → **Xwayland** → Weston, e il suo container non vede nemmeno `/dev/fb0` |
...
| 4 | immagine di boot nel container | **✓ chiuso il 24-09, ma per un'altra strada.** Il maintainer ha posto il vincolo «nell'host non puoi toccare nulla, tutto deve essere fatto con chiamate dBus dal container»: le tre cose sull'host sono state **cancellate** e la chiamata la fa il runtime (`sws-web/src/launcher_dbus.rs`). Sul pannello: `esito=applicata`, `GetBackgroundImage → "boot.png"`, e **dopo spegnimento e riaccensione il maintainer ha visto la propria immagine** sullo schermo. Serviva `UserNS=keep-id`, o D-Bus rifiuta la connessione del container rootless |
| 5 | Q55, scrittura dal viewer LVGL | **✓ collaudato il 24-09**, appena LVGL è andato a schermo. Pagina di prova con uno slider su un tag interno: il maintainer ha trascinato lo slider sul touch e il tag è passato da `0`/`Uncertain` a `19` e poi `69`, qualità **Good** — più scritture in fila, tutte arrivate, e il viewer è rimasto vivo (stesso pid). La combinazione che Q55 dice bloccarsi — una PUT che riceve **200** dentro il binario del viewer — **completa**, col thread di rete del 20-09. Provata anonima: il progetto non ha utenti, quindi il caso **con token** resta da fare |
