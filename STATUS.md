...
**Last session**: 2026-05-19 (sessione 26 quad → S-27 UX + bonus pack; sessione 27 → Playwright E2E + WS tag write bidirezionale + BL-005 OPC-UA step 1+2; **sessione 28 → BL-005 OPC-UA step 3 (browse) + step 4 (writes via TagWriteBus)** — BL-005 chiusa eccetto security policies > None / Euromap / reverse browse)
...
**Last commit**: `89d95a3 feat(BL-005): OPC-UA writes (step 4) + server browse (step 3)`. 1 commit della sessione 28 NON ancora pushato a `origin/main` (i 4 della sessione 27 sono su origin da `f36881d`).

### Handoff per la prossima sessione

1. **`git pull` su `main`** + verifica il commit sessione 28 (sopra).
...
5. **Prossimi step raccomandati** (uno qualunque, in ordine di complessità crescente):
   - **BL-005b — Euromap 77/83 auto-discovery** (~3h): companion-spec detection sul namespace browse + import massivo nodi con nomi standard. Vedi BL-005b nello storico.
   - **BL-005c — OPC-UA security policies > None** (~3-4h): aggiungere supporto a `Basic256Sha256` (cert handshake + keypair). Forwardabile dato che lo schema YAML già accetta il campo.
   - **6.4 follow-up — riconnessione WS automatica** con backoff esponenziale (oggi single attempt, comment "TODO" in `tagStream.ts`).
   - **OPC-UA reverse browse**: il browse handler oggi solo `BrowseDirection::Forward`; estendere per references inbound (utile per inspector address space).
