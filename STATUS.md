...
**Last session**: 2026-05-19 (sessione 26 quad: S-27 UX + bonus pack — aspect-ratio resize, counter Prometheus, test funzioni, backup `.bak/`, export/import YAML, integration tests; poi **sessione 27: Playwright E2E + WS tag write bidirezionale + BL-005 OPC-UA client step 1+2**)
...
**Last commit**: `d9a1913 feat(BL-005): OPC-UA client plugin — Phase 4 step 1+2`. 3 commit della sessione 27 NON ancora pushati a `origin/main` (i 7 della sessione 26 sono già su origin): `ab62ad4` Playwright e2e, `dcd934a` /ws/tags bidir, `d9a1913` BL-005 OPC-UA.

### Handoff per la prossima sessione

1. **`git pull` su `main`** + verifica i 3 commit sessione 27 (sopra).
...
5. **Prossimi step raccomandati** (uno qualunque, in ordine di complessità crescente):
   - **BL-005 step 3 — OPC-UA browse endpoint** (~2h): nuovo `POST /api/sources/opcua/browse` + bottone "Sfoglia server" nell'OpcUaSourceCard. Aprire una connessione temporanea, browsare l'address space ricorsivamente, ritornare l'albero per la UI.
   - **BL-005 step 4 — OPC-UA writes** (~1h): instradare `TagWriteBus → async_opcua::Client::write` per le scritture verso il server, mirror della logica Modbus.
