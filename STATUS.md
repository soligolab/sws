...
**Last session**: 2026-05-19 (sessione 29 → BL-005 final; **sessione 30 → 2 nuovi template (opcua-demo, grid-playground) + refresh standards su demo-items e casa-locale + rimozione progetto di default da dev.sh**)
**Current phase**: Phase 2. Demo working out-of-the-box su fresh clone, import/export progetto per backup/condivisione, pannello log live + persistenza su disco, gestione utenti multi-account. Da S-22→S-26 il runtime può servire la SPA da sé (`--www`), puntare a un runtime remoto via `VITE_RUNTIME_URL`/`localStorage`, spawnare un browser kiosk (`--kiosk-browser`), e la stessa SPA bundle può saltare tra runtime diversi. S-27 ha chiuso 3 UX gap nell'editor: drag&drop con grouping nel tree, context menu tasto destro, righelli con guide draggabili + snap. Bonus: `/metrics` Prometheus ora reale (CPU/RAM/disco/tag/allarmi sample-on-scrape), test integrazione `/api/system` + metrics (41 unit test verdi), export/import singola pagina YAML (oltre al ZIP intero). **S-30**: avvio fresh clone non parte più con un progetto auto-aperto — la WelcomeScreen sceglie tra 4 template (`demo-items` 4 pagine, `casa-locale` 5 pagine MQTT, **`opcua-demo`** 2 pagine BL-005, **`grid-playground`** 2 pagine grid 4×4 con sub-cell fino a 4 livelli).
**Last commit**: `feat(templates): refresh standards, +2 templates (opcua/grid), remove default project`. Più commit recenti (TESTING_GUIDE doc + il presente refresh templates) NON ancora pushati a `origin/main` — verificare con `git log --oneline origin/main..HEAD`.

### Handoff per la prossima sessione

1. **`git pull` su `main`** + verifica i 2 commit non pushati (sessione 29 + sessione 30 templates refresh).
2. **`git push origin main`** quando confermi tutto OK.
3. **Smoke-test sessione 30 (~10 min)**:
   - `./scripts/dev.sh` → editor su 5173. La WelcomeScreen NON deve mostrare un progetto "dev" pre-esistente (rimosso). Deve esserci la sezione template con 4 voci: `Demo items`, `Casa Locale (MQTT)`, `OPC-UA Demo (BL-005)`, `Grid Playground`.
   - Crea un nuovo progetto da **Grid Playground** → apri la pagina "Griglia 4×4". La cella (0,0) ha 4 livelli di sub-cell annidate (cols → rows → cols → rows). Click su "Toggle pompa" (cella 2,1) → il LED nel livello 4 e il simbolo pump nel livello 1 reagiscono. Vai in Page 2 "Controlli", scegli un colore preset → l'anteprima fade su 400 ms (transition_duration_ms) e ritornando a Page 1 anche il rect in cella (1,3) ha cambiato fill.
   - Crea un nuovo progetto da **OPC-UA Demo (BL-005)**. Senza simulatore attivo i tag rimangono `Bad` (dot rossi) — è OK. Avvia un simulatore (`npx -y node-opcua-server` su 4840) e in pochi secondi gauge + Trend si muovono. Apri ConfigView → OPC-UA → "🤖 Rileva Euromap" per testare l'auto-detect (richiede però che il simulatore esponga davvero CycleTime/MachineState/…). Vedere `examples/templates/opcua-demo/SETUP.md`.
   - Crea un nuovo progetto da **Demo items** o **Casa Locale**: il navigation header è ora in un gruppo `g_nav` con `locked: true` → in editor mode i nav button NON sono trascinabili (lucchetto 🔒 accanto al tipo nel LeftPanel) e cliccarli a runtime funziona come prima.
4. **Smoke-test S-27 + bonus pack della sessione 26-29** (~15 min, se hai tempo):
   ...
   - **Metrics Prometheus**: `curl -k https://localhost:8443/metrics` → vedi gauge `sws_uptime_seconds`, `sws_tag_count`, `sws_cpu_usage_pct`, `sws_memory_used_bytes`, `sws_disk_used_bytes`. Esegui un paio di tag-read e una scrittura: `sws_http_requests_total{path,method,status}` deve incrementare. Triggera un alarm: vedi `sws_alarm_transitions_total{direction="activated",severity="..."}` aumentare. Lancia uno script dal pannello 🧪: vedi `sws_script_exec_total{endpoint="run",status="ok"}` aumentare.
   - **Aspect-ratio resize**: seleziona un oggetto rettangolare (non linea/grid/ruotato), tieni **Shift** e trascina un angolo (`tl/tr/bl/br`) → mantiene il rapporto larghezza/altezza iniziale.
   - **Pannello 🧪 test funzioni** (RuntimeView): solo se il progetto ha funzioni definite. Click sul bottone 🧪 bottom-left → dialog con dropdown funzioni, input editabili per ogni parametro (vuoto = usa default), bottone "▶ Esegui". Output finisce nel toast esistente.
   - **Backup admin** (ConfigView tab Backup, solo Admin): click "+ Backup adesso" → snapshot in `.bak/<ts>/` di project.yaml + synoptics + users.yaml. Lista mostra newest-first con dimensione. "Ripristina" sovrascrive il progetto live + ricarica le pagine. **Auto-backup**: per testare il loop avvia `./scripts/dev.sh runtime` con `SWS_AUTO_BACKUP=...` oppure passa CLI args `--auto-backup-interval-minutes 5 --auto-backup-retention 10` direttamente; dopo 5 min vedrai un nuovo backup comparire.
   - **WS tag write bidirezionale** (6.4): in RuntimeView clicca un pulsante che scrive un tag → niente HTTP PUT visibile in DevTools/Network, ma il valore propaga via `/ws/tags`. Verifica nel WS inspector: dovresti vedere frame `{"type":"write",…}` in uscita e `{"type":"ack","ok":true,…}` in ingresso. Falla quando offline = fallback HTTP PUT.
   - **Playwright e2e** (9.1): solo locale, una tantum `npx playwright install chromium`, poi `./scripts/dev.sh both` + `cd sws-editor && pnpm test:e2e`. Dovrebbero passare entrambi i test (login golden path + login form errato). Recipes dettagliate in `scripts/README.md` §End-to-end tests.
   - **OPC-UA client** (BL-005 step 1+2): avvia un simulatore (`npx node-opcua-server-example` o Prosys OPC UA Simulation Server). In Configurazione → Protocolli → "+ Aggiungi OPC-UA" → endpoint URL + Nodo (es. `ns=1;s=Temperature` per node-opcua) + tag. Salva. Crea una sezione Trend/Gauge bindata al tag → vedi valori live in View. Documentazione completa: `docs/OPCUA_SETUP.md`.
   - **OPC-UA browse** (step 3): nella card OPC-UA bottone "🔍 Sfoglia server" → tree modale; click su Object si espande lazy (chiamata browse a quel NodeId), tick una Variable, "Importa" → riga aggiunta nella tabella nodi con `display_name` come description.
   - **OPC-UA writes** (step 4): metti un pulsante o slider canvas bindato a un tag OPC-UA scrivibile; in RuntimeView lo write va via TagWriteBus → plugin scrive via `session.write()` sul NodeId mappato; verifica nei log del simulatore che il valore sia cambiato. Echo immediato in TagDb quando lo status del server è Good.
   - **OPC-UA security policies** (BL-005c): nella card OPC-UA cambia "Security policy" da `None` a `Basic256Sha256`. Salva. Sul simulatore la prima connessione fallisce con cert rejected — apri il simulatore Prosys (o equivalente), vai in Certificates → Rejected, trust il cert SWS; il prossimo retry (entro 5s) deve riuscire. Cert + chiave persistono in `.run/projects/<proj>/.opcua-pki/<source-id>/`.
   - **Euromap auto-detect** (BL-005b): se il server espone variabili con nomi canonici Euromap 77/83 (es. `CycleTime`, `MachineState`), clicca "🤖 Rileva Euromap" nella card. Il modal scansiona, presenta una tabella con matches pre-selezionati; toggle "Auto-crea tag SWS" decide se anche i tag vengono creati. Importa → righe aggiunte alla tabella nodi + tag corrispondenti. Per testare senza Euromap reale, fai un mock: aggiungi nodi `ns=2;s=CycleTime`, `ns=2;s=MachineState` sul tuo simulatore.
...
5. **Prossimi step raccomandati** (uno qualunque, in ordine di complessità crescente):
   - **6.4 follow-up — riconnessione WS automatica** con backoff esponenziale (oggi single attempt, comment "TODO" in `tagStream.ts`).
   - **OPC-UA vendor-curated trust list** (UI per accettare/rifiutare server cert per-source; oggi `trust_server_certs(true)`).
   - **OPC-UA historical reads** (`HistoryRead` service) — utile per backfill grafici Trend al primo open.
   - **S-25** test su PX30 fisico (richiede hardware) — unico blocker tra noi e "deploy unattended pronto".
   - **Bundle splitting** Vite: il chunk principale è ~900 KB (273 KB gzip). `manualChunks` per separare codemirror / react / runtime.

### Cosa è andato online in queste sessioni (in ordine di commit)
- (sessione 2026-05-19, sessione 30) **2 nuovi template + refresh standards + rimozione progetto di default** — `examples/templates/` ora ha 4 voci: `demo-items` (refresh), `casa-locale` (refresh), `opcua-demo` (nuovo, BL-005 showcase), `grid-playground` (nuovo, grid 4×4 con sub-cell fino a 4 livelli). **Refresh standards** sui due template esistenti: ogni pagina ora dichiara `width:` / `height:` espliciti; nav header (button + titolo) raggruppato in `g_nav` (e `g_header` quando distinto) con `locked: true` per immune-ai-drag accidentali; `transition_duration_ms` sui widget bindati (gauge counter + symbol pump in demo-items; rect/ellipse bindati a `demo.fill_color` in Page 4). Versioni bumpate a `0.2.0`; `meta.name` di demo-items corretto da `dev` a `demo-items`; description del `template.yaml` aggiornate. **opcua-demo** (2 pagine, 5 tag, 2 alarm): `SourceDef::OpcUaClient` su `opc.tcp://localhost:4840` con 2 nodi `ns=1` (Temperature, Pressure) e 3 nodi `ns=2` Euromap-77 canonici (CycleTime, MachineState, MachineReady); Page 1 = 2 gauge + cycle/state/ready + trend `extra_tags`; Page 2 = tabella NodeId→Tag→Value live; `SETUP.md` con recipe `node-opcua` e Prosys, gotcha trust handshake Basic256Sha256. **grid-playground** (2 pagine, 6 tag, 1 alarm, 3 functions): un `grid` 4×4 (760×600) con `grid_show_borders: true` che esercita figli di tipo rect/ellipse/text/gauge/led/symbol/button/slider/progress_bar/trend, una merge cell `colspan: 2`, varie celle con `sub` shallow, e UNA cella con 4 livelli di `sub` nidificati (cols → rows → cols → rows). Page 2 controlli con color picker + button "counter=95" per triggerare alarm. **dev.sh** non seeda più `.run/projects/dev/` e non passa `--project`: i due rami (`runtime`, `both`) lanciano il runtime con solo `--projects-root` + `--templates-root`; la WelcomeScreen sceglie. Heredoc YAML legacy mantenuto come `: <<'LEGACY_FALLBACK_YAML'` di documentazione, NON eseguito. `.run/projects/dev/` cancellato. Validazione: `cargo check` + `pnpm build` verdi; smoke `Project::load` + `GET /api/synoptics/:name` per pagina su tutti e 4 i template → 4/4 progetti caricano (76 tag / 10 alarm / 5 functions in totale), 13/13 pagine ritornano 200.
- (sessione 2026-05-19, sessione 29) **BL-005 final: security policies, Euromap detect, reverse browse** — chiude tutto il backlog OPC-UA modulo trust list curation / historical reads / reverse-connect. **BL-005c security**: `parse_security_policy` mappa la stringa YAML su `SecurityPolicy` (None / Basic128Rsa15 / Basic256 / Basic256Sha256 / Aes128Sha256RsaOaep / Aes256Sha256RsaPss); ignote → None con warning. Non-None pairs con `SignAndEncrypt`. Helper `build_client_builder` centralizza `create_sample_keypair(true)` + `trust_server_certs(true)` + `pki_dir`. Cert + chiave persistite in `<project>/.opcua-pki/<source-id>/`, isolated per-source. `SourceSupervisor::set_pki_root` chiamato da open_project (sia WelcomeScreen che `--project` legacy). ConfigView dropdown security policy ora abilitato per tutte le voci. **BL-005b Euromap**: `detect_euromap` BFS (cap 500 nodi / 4 livelli sotto ObjectsFolder), match case-insensitive su 11 nomi canonici (EM77 IM 8 vars + EM83 TCU 3 vars). `POST /api/sources/opcua/detect-euromap` (Operator+). UI: bottone "🤖 Rileva Euromap" in OpcUaSourceCard → modal scan auto, pre-select matches non importati, "Auto-crea tag SWS suggeriti" toggle che crea anche i tag `<sid>.<suffix>` al click Importa. **Reverse browse**: `browse_one_level` accetta `BrowseDir { Forward, Inverse, Both }`. Browse endpoint accetta `direction` + `security_policy` ovveride. Tests +3 (sws-plugin-opcua 5→8, workspace 50→53). Docs `OPCUA_SETUP.md` riscritte con sezioni Security + Euromap + Reverse direction; deferred list ridotta a trust list / historical reads / reverse-connect.
- (sessione 2026-05-19, sessione 28) **BL-005 OPC-UA step 3+4: server browse + writes via TagWriteBus** — chiude la demo operator-facing OPC-UA. **Step 4 writes**: `sws-plugin-opcua::run` ora prende `&Arc<TagWriteBus>`; per ogni nodo registra un sender mpsc, un writer task converte TagValue → Variant (Bool→Boolean, Int→Int64, Float→Double, Str→String) e chiama `session.write(&[WriteValue { node_id, attribute_id: Value, value }])`. Status `Good` ⇒ echo immediato in TagDb (UI vede il cambio senza aspettare il prossimo publish). Su disconnect il writer è abortito + `bus.unregister_many` rilascia le route; reconnects ri-registrano. **Step 3 browse**: nuovo helper `browse_one_level(cfg, parent_node_id)` apre sessione temporanea, browse Forward + HierarchicalReferences sotto NodeId (o ObjectsFolder se None), ritorna `Vec<BrowsedNode>{node_id,browse_name,display_name,node_class}`, chiude. Endpoint `POST /api/sources/opcua/browse` (Operator+) con masked-password sentinel resolution. **UI**: bottone "🔍 Sfoglia server" nell'`OpcUaSourceCard` → `OpcUaBrowseModal` con tree lazy-loaded (Object click → espande, Variable → checkbox, già-importati greyed-out). 2 nuovi test (`tag_value_to_variant_roundtrip_*`), workspace 48 → 50. Docs `OPCUA_SETUP.md` riscritte: nuove sezioni "Writes back to the server" + "Browse the server"; deferred list ridotta a security>None / Euromap / reverse browse.
- (sessione 2026-05-19, sessione 27) **BL-005 OPC-UA client — Phase 4 step 1+2** — primo slice del demo industriale. (1) `sws-core`: nuova variant `SourceDef::OpcUaClient(OpcUaClientConfig)` con endpoint_url, security_policy ("None" wired, altre stored per forward-compat), auth (Anonymous | UsernamePassword con password_env che vince su password), subscription_interval_ms, Vec<OpcUaNodeMapping>. (2) `sws-plugin-opcua` upgradato da placeholder: ClientBuilder + connect_to_endpoint_directly + DataChangeCallback. Il callback synchronous ship `(NodeId, DataValue)` su mpsc → dispatcher tokio drains in TagDb (worker OPC-UA non await mai). Map per Bool/int8-64/Float/Double/String/LocalizedText; unsupported variants → Uncertain. Disconnect = tag Bad fino a prossimo data-change. 3 unit test. (3) `sws-web::source_supervisor` dispatcha la variant; workspace dep `async-opcua` con feature `client`. (4) ConfigView: nuovo OpcUaSourceCard (endpoint URL, security policy picker, subscription interval, auth toggle, NodeId table con TagInput + quick-create). "+ Aggiungi OPC-UA" rimpiazza "prossimamente". LeftPanel SOURCES pillola verde "OPC-UA". (5) `docs/OPCUA_SETUP.md` con setup, 2 simulator recipes (node-opcua, Prosys), smoke-test, lista esplicita di ciò che NON c'è (writes, browse, security policies > None, Euromap = step 3/4/5/BL-005b).
- (sessione 2026-05-19, sessione 27) **WS tag write bidirezionale (task 6.4)** — `/ws/tags` ora accetta frame `{"type":"write","tag":...,"value":...,"req_id":?}`. Role-gated Operator+ (mirror HTTP `PUT /api/tags/:id`), instrada via `TagWriteBus` con fallback TagDb. Risposta server: `WriteAck` sullo stesso socket. Backend: handler split via `futures-util` Sink/Stream + tre task (forwarder mpsc, broadcast subscriber, inbound dispatcher) — frame in uscita serializzati su una sola queue per evitare race sul framer. Frontend: nuovo `tryTagWriteWs(tag, value)` helper; `RuntimeView.handleWriteTag` prova WS, fallback `api.writeTag()` solo se socket non aperto. Reader esistente ignora ack frames (logga failure su console).
- (sessione 2026-05-19, sessione 27) **Playwright e2e (task 9.1)** — `@playwright/test` dev-dep + due specs in `sws-editor/e2e/`: (1) login golden path (admin/admin → "Rettangolo" → "☰ Menu → Salva tutto" → reload → conta rect blu fill `#3b82f6`), (2) login form errore "Credenziali non valide". Config: chromium project, `baseURL https://localhost:5173`, `ignoreHTTPSErrors: true`, `trace: retain-on-failure`. **No webServer** in config — l'operatore lancia `dev.sh both` separatamente (duplicare cert/seed bootstrap sarebbe più fragile). `pnpm test:e2e` + `:ui`. Artefatti `test-results/` + `playwright-report/` gitignored. `scripts/README.md` §End-to-end documenta il run sequence.
- (sessione 2026-05-19, sessione 26-ter) **Backup automatico progetto + restore admin (task 7.2)** — snapshot point-in-time del progetto sotto `<project>/.bak/<UTC-ts>/` che coprono project.yaml, synoptics/, users.yaml. Due trigger: (a) **loop background** quando il runtime parte con `--auto-backup-interval-minutes N` (default 0 = disabilitato); `--auto-backup-retention K` cappa il numero di snapshot (default 20). Prima tick saltata per non snapshottare a startup. I/O sotto `spawn_blocking`. (b) **REST admin**: `GET/POST /api/backups`, `DELETE /api/backups/:name`, `POST /api/backups/:name/restore`. Path param sanitizzato da `safe_backup_name` contro `..`/`/`/length. **ConfigView** nuova tab "Backup" (admin-only) con tabella newest-first (nome, creato, dimensione) + bottoni Backup adesso / Aggiorna / Ripristina / Elimina; restore ricarica project + pages immediatamente. 4 nuovi unit test (sws-web 8 → 12).
- (sessione 2026-05-19, sessione 26-bis) **Aspect-ratio resize + Prometheus counters + script test panel** — 3 task indipendenti chiusi insieme. (1) **Task 1.5**: Shift + drag su un corner handle (`tl/tr/bl/br`) blocca l'aspect ratio iniziale dell'oggetto. Driver axis = quello che si è mosso di più in unità width-equivalent (`dySigned * aspect`); l'altro asse è derivato. Anchor del corner opposto preservato. Mid-edge handle ignorano Shift (una sola dim cambia). Documentato in ShortcutHelp. (2) **Task 3.4b**: counter Prometheus aggiuntivi — `sws_script_exec_total{endpoint,status}` (in `exec_script` + `run_function`), `sws_alarm_transitions_total{direction,severity}` (nel dispatcher webhook, single chokepoint per ogni transizione, activated + recovered), `sws_http_requests_total{path,method,status}` (middleware `track_http_metrics` con `MatchedPath` per cardinalità bounded — `/metrics` + `/health` esclusi). `sws-runtime` ora dipende da `metrics` workspace. (3) **Task 6.3**: pannello "🧪 Test funzioni" floating bottom-left in RuntimeView, visibile solo se il progetto ha funzioni. Dropdown funzioni + input editabili per ogni param (vuoto = default), Run riusa `handleScript` esistente → output ai toast + counter `sws_script_exec_total` incrementa. Type coercion mirror del default declarato (bool/number/string). Override per-funzione tenuti in component state per iterazione veloce.
